...
void run_result::print(const time_t &test_start, std::ostream &o) const {
  ...
  ss << duration_cast<microseconds>(start_time - test_start) << ", "
     << duration_cast<microseconds>(stop_time - test_start) << ", " << is_write
     << ", " << is_abort << ", \"" << abort_string << "\", "
     << is_protocol_error << ", " << is_fatal_error << ", " << slept_for << ", "
     << desired_delay << ", " << effective_delay << endl;
  o << ss.str();
}
void run_result::read(const time_t &test_start, std::istream &i) {
  ...
  assert(start_offset.count() != 0 && stop_offset.count() != 0 &&
