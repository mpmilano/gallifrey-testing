...
int main(int argc, char **argv) {
  ...
  for (std::size_t i = 0; i < results.size();) {
    ...
    struct at_scope_end {
      std::size_t &i;
      std::size_t &adjust_window_by;
      ~at_scope_end() { i += adjust_window_by; }
    };
    at_scope_end ase{i, adjust_window_by};
    (void)ase;
    if (bin_size)
      --bin_size;
    if (bin_size == 0 && i == results.size() - 1)
      break;
    else if (bin_size == 0) {
      // std::cout << "outlier: ";
    ...
    if (i + bin_size >= results.size()) {
      // std::cout << i << " " << bin_size << " " << results.size() <<
      // std::endl;
    }
    assert(i + bin_size < results.size());
    assert((results[bin_size + i].stop_time - results[i].stop_time).count() >
           0);
    ...
    if (bin_size > 0 && bin_size * params.log_every_n *
                                duration_cast<duration_t>(1s).count() /
                                segment_duration.count() !=
                            0) {
      bin_averages.emplace_back(
          Frequency{bin_size * params.log_every_n *
                    duration_cast<duration_t>(1s).count() /
                    segment_duration.count()},
