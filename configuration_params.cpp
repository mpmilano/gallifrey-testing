...
bool same_run(const configuration_parameters &p1,
  ...
  return p1.strong_ip == p2.strong_ip &&
         p1.strong_relay_port == p2.strong_relay_port &&
         p1.causal_relay_port == p2.causal_relay_port &&
         p1.client_freq == p2.client_freq &&
         ...
         p1.percent_causal == p2.percent_causal &&
...
std::ostream &operator<<(std::ostream &o, const configuration_parameters &p) {
  ...
  return o << string_of_ip(p.strong_ip) << cs << p.strong_relay_port << cs
           ...
           << p.client_freq << cs << p.starting_num_clients << cs
           << p.increase_clients_freq << cs << p.test_duration << cs
           << p.percent_dedicated_connections << cs << p.percent_causal << cs
...
std::istream &operator>>(std::istream &i, configuration_parameters &p) {
  ...
  i >> strong_ip >> cs >> p.strong_relay_port >> cs >> causal_ip >> cs >>
      p.causal_relay_port >> cs >> p.client_freq >> cs >>
      p.starting_num_clients >> cs >> p.increase_clients_freq >> cs >>
  ...
  std::cout << strong_ip << std::endl << causal_ip << std::endl;
